...
public:
  // конструктор по умолчанию: список элементов пока пуст
    AirportCounter():data{0}, Airports_quantity(static_cast<size_t>(TAirport::Last_)) {}
  ...
  Items GetItems() const{
      array<Item, static_cast<uint32_t>(TAirport::Last_)> a;
      for (size_t i=0 ; i < Airports_quantity; ++i) {
          a[i] = {static_cast<TAirport>(i), data[i]};
      }
      return a;
  }
private:
  array <size_t, static_cast<size_t>(TAirport::Last_)> data;
  size_t Airports_quantity;
